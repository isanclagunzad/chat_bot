        ...
        <div class="card-body">
            <div class="overflow-auto" id="convo-box">
                <div id="suggestion-list" class="my-4 px-5">
                    <?php 
                    $qry = $conn->query("SELECT keyword, COUNT(*) as count FROM `keyword_list` GROUP BY keyword ORDER BY count DESC");
                    $grouped_suggestions = [];
                    while($row = $qry->fetch_assoc()){
                        $suggestion = $row['keyword'];
                        $grouped_suggestions[$suggestion] = $row['count'];
                    }

                    $suggestions = $_settings->info('suggestion') != '' ? json_decode($_settings->info('suggestion')) : [];
                    ?>
                    <a href="javascript:void(0)" class="w-auto rounded-pill bg-transparent border px-3 py-2 mb-2 text-decoration-none text-reset suggestion"><?= $suggestions[0] ?></a>
                    <?php //endforeach; ?>
                    <?php foreach($grouped_suggestions as $suggestion => $count):
                         if ($suggestion === $suggestions[0]) {
                            continue;
                        }
                    ?>
                            <a href="javascript:void(0)" class="w-auto rounded-pill bg-transparent border px-3 py-2 mb-2 text-decoration-none text-reset suggestion"><?= $suggestion ?></a>
                    <?php endforeach; ?>
                </div>
